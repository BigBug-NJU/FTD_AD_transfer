...
def validate(val_loader, model, criterion, sets):
    ...
    with torch.no_grad():
        ...
        for batch_id, (volumes, target) in enumerate(val_loader):
            ...
            output = model(volumes)
            if output.ndim != 2:
                print("varfy output:", output.shape, "-->", target.shape)
                continue
            loss = criterion(output, target)
...
def save_checkpoint(state, is_best, filename='org_checkpoint.pth.tar'):
    torch.save(state, filename)
    if is_best:
        shutil.copyfile(filename, 'org_model_best.pth.tar')
...
# e3d_adam
# python org_transfer.py --gpu_id 4 5 --data_root=../../data/ResImg/ --num_classes=3 --pretrain_path=./pretrain/resnet_50_23dataset.pth --batch_size=4 --n_epochs=200 > hjj_org.log